...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Selectables are meant to be tightly packed together with no click-gap
    ...
    // Modify ClipRect for the ItemAdd()
    const float backup_clip_rect_min_x = window->ClipRect.Min.x;
    const float backup_clip_rect_max_x = window->ClipRect.Max.x;
    if (span_all_columns)
    {
    ...
    // Update NavId when clicking or when Hovering
    ...
    const float rounding = 2.f; // Закругление 2px
    ...
    // Hover/active effect with animation
    if (hover_anim > 0.0f)
    {
