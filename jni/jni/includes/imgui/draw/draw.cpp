...
namespace draw {
    ...
    // Диапазон глифов для иконочного шрифта hitmarker (Private Use Area)
    ...
    io.Fonts->AddFontDefault(&font_cfg);
        ...
        // Захватываем ввод сразу, т.к. меню открыто при старте
    ...
    void shutdown() {
        ...
        if (native_window) {
            // Используем ANativeWindowCreator::Destroy вместо ANativeWindow_release,
            // т.к. окно было создано через ANativeWindowCreator::Create
