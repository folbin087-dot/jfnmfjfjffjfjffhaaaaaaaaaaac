...
namespace style {
    ...
    static ImRect popup_rect = ImRect(0, 0, 0, 0); // Track popup bounds for proper click detection
    static std::string picker = "";                // Color picker state
    ...
    // Combo box variables
    static ImVec2 cmb_pos = ImVec2(0, 0);
    static float cmb_w = 0.f;
    ...
    void tick() {
        // Reset click state at start of each frame
        clicked = false;
        // НЕ сбрасываем frame_clicked здесь - он должен сбрасываться в конце кадра
        static float lt = 0.f;
    ...
    // Вызывается в конце кадра для сброса состояния кликов
    void end_frame() {
        frame_clicked = false;
    }

    // Helper to check if click should be consumed
    static bool consume_click() {
        if (frame_clicked) return false;
        frame_clicked = true;
        clicked = true;
        return true;
    }
    ...
    void close() { 
        active_popup = ""; 
        popup_open = false; 
        popup_rect = ImRect(0, 0, 0, 0);
        popup_interacting = false;
        picker = "";
        frame_clicked = false;
    }

    void set_popup_rect(const ImRect& rect) { popup_rect = rect; }
    ...
    void popups() {
            ...
            // Только закрываем popup если кликнули ВНЕ области popup'а
            // И только если popup полностью отрисован (pa > 0.9f)
            if (pa > 0.9f && ImGui::IsMouseClicked(0) && !frame_clicked) {
                ImVec2 mp = ImGui::GetIO().MousePos;
                // Проверяем что мышь вне области popup'а
                if (popup_rect.GetWidth() > 0 && popup_rect.GetHeight() > 0) {
                    // Добавляем небольшой отступ для удобства
                    ImRect expanded_rect = popup_rect;
                    expanded_rect.Expand(10.0f);
                    if (!expanded_rect.Contains(mp)) {
                        active_popup = "";
                        popup_open = false;
                        popup_rect = ImRect(0, 0, 0, 0);
                        frame_clicked = true; // Потребляем клик
                    }
                }
            }
        }
...
namespace widgets {
    ...
    bool checkbox(const char* name, bool* v, float a) {
        ...
        // Only allow interaction if no popup is open
        bool hov = ImGui::IsMouseHoveringRect(r.Min, r.Max) && !popup();
        bool press = hov && ImGui::IsMouseClicked(0) && !frame_clicked;
        if (press) { 
            *v = !*v; 
            frame_clicked = true;
            clicked = true; 
        ...
        // Smooth animation for check state
        float ca = anim(ids + "_c", *v ? 1.f : 0.f, 12.f);
        // Hover animation
        float ha = anim(ids + "_h", hov ? 1.f : 0.f, 14.f);

        ImDrawList* dl = w->DrawList;

        // Panel background with hover effect
        dl->AddRectFilled(r.Min, r.Max, col(clr::panel, a * 0.5f * ha));
        if (ha > 0.01f) {
            dl->AddRectFilledMultiColor(
                r.Min, ImVec2(r.Min.x + ww * 0.25f, r.Max.y),
                col(clr::accent, a * ha * 0.1f), col(clr::accent, 0.f),
                col(clr::accent, 0.f), col(clr::accent, a * ha * 0.1f));
        }
        ...
        ImVec4 bc = anim_col(ids + "_bc", *v ? clr::accent : clr::border_light, 12.f);
        ...
        ImVec4 tc = anim_col(ids + "_tc", *v ? clr::text : clr::text_dim, 10.f);
    ...
    bool combo(const char* name, int* v, const std::vector<const char*>& items, float a) {
            ...
            bool press = hov && ImGui::IsMouseClicked(0) && !other && !frame_clicked;
            ...
            if (oa > 0.01f && cmb_ptr == v) {
                ...
                // Set popup rect for proper click detection
                set_popup_rect(ImRect(pmin, pmax));
                ...
                for (int i = 0; i < (int)items.size(); i++) {
                    ...
                    if (ihov && ImGui::IsMouseClicked(0) && !frame_clicked && open && oa > 0.7f) {
                        *v = i;
                        active_popup = "";
                        popup_open = false;
                        frame_clicked = true;
    ...
    void colorpick(const char* name, ImVec4* v, float a) {
        ...
        if (oa > 0.01f && cp_ptr == v) {
            ...
            float p = 8.f * S;
            float gap = 6.f * S;
            float close_btn_sz = 16.f * S;
            float pw = p + sv_sz + gap + hue_w + p;
            float ph = p + sv_sz + p + close_btn_sz + p; // Увеличиваем высоту для кнопки закрытия

            ImVec2 pmin(r.Max.x - pw, r.Max.y + 4.f * S);
            ImVec2 pmax(pmin.x + pw, pmin.y + ph);

            // Устанавливаем область popup для правильного определения кликов
            set_popup_rect(ImRect(pmin, pmax));
            ...
            // КНОПКА ЗАКРЫТИЯ (X)
            ImVec2 close_min(pmax.x - close_btn_sz - p, pmin.y + p);
            ImVec2 close_max(close_min.x + close_btn_sz, close_min.y + close_btn_sz);
            bool close_hov = ImGui::IsMouseHoveringRect(close_min, close_max) && oa > 0.7f;
            bool close_click = close_hov && ImGui::IsMouseClicked(0) && !frame_clicked;
            ...
            ImU32 close_col = close_hov ? col(clr::accent, oa) : col(clr::text_dim, oa);
            fg->AddRectFilled(close_min, close_max, col(clr::widget, oa * (close_hov ? 0.8f : 0.3f)));
            fg->AddRect(close_min, close_max, close_col);
            
            // Рисуем X
            float cross_pad = 4.f * S;
            fg->AddLine(
                ImVec2(close_min.x + cross_pad, close_min.y + cross_pad),
                ImVec2(close_max.x - cross_pad, close_max.y - cross_pad),
                close_col, 1.5f
            );
            fg->AddLine(
                ImVec2(close_max.x - cross_pad, close_min.y + cross_pad),
                ImVec2(close_min.x + cross_pad, close_max.y - cross_pad),
                close_col, 1.5f
            );

            if (oa > 0.1f) {
                ImVec2 sv_min(pmin.x + p, pmin.y + p + close_btn_sz + p);
                ...
                if (io.MouseDown[0] && !frame_clicked) {
                    if (picker.empty()) {
                        if (mp.x >= sv_min.x && mp.x <= sv_max.x && mp.y >= sv_min.y && mp.y <= sv_max.y) {
                            picker = ids + "_sv";
                            frame_clicked = true;
                        }
                        else if (mp.x >= hue_min.x && mp.x <= hue_max.x && mp.y >= hue_min.y && mp.y <= hue_max.y) {
                            picker = ids + "_hue";
                            frame_clicked = true;
                        }
                    }
