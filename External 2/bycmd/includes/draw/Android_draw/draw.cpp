...
bool ImGui_init() {
    ...
    // Load menu fonts with Cyrillic support
    ...
    // Load ESP font with FULL glyph ranges including weapon icons
    // Weapon icons use special characters from the icon font
    static const ImWchar icon_ranges[] = {
        0x20, 0x7E,  // Basic Latin (space to ~) - covers all weapon icon letters
    ...
    icon_font_cfg.GlyphRanges = icon_ranges;
    espFont = io.Fonts->AddFontFromMemoryTTF(esp, sizeof(esp), 24, &icon_font_cfg, icon_ranges);

    // Load weapon icon font from Allah project
    weaponIconFont = io.Fonts->AddFontFromMemoryTTF(WeaponImages, sizeof(WeaponImages), 24, nullptr, io.Fonts->GetGlyphRangesCyrillic());
